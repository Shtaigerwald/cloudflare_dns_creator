...
func addUniqueDomainInList(domainList *[]string, domain string) {
    ...
    if !containStringInList(*domainList, baseDomain) && !containStringInList(*domainList, wildcardDomain) {
...
func containStringInList(s []string, str string) bool {
    for _, v := range s {
        if v == str {
            return true
        }
